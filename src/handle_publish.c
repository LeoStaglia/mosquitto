...
int handle__publish(struct mosquitto *context)
{
    ...
    printf( "Received PUBLISH from %s (%d, %d, %d, '%s', ... (%ld bytes))", context->id, dup, msg->qos, mid, msg->topic, (long)msg->payloadlen);


    if(strcmp(context->id,"JavaWS") != 0){
        printf("ClientID : %s\n",context->id);
        mqtt_plus_publish(msg->topic,context->id,msg->qos,msg->payload,(long) msg->payloadlen);
    }
