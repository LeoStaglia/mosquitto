...
char *webserver_port;
bool is_distributed_pub;
bool is_distributed_sub;
...
int config__parse_args(struct mosquitto__config *config, int argc, char *argv[])
{
	...
	for(i=1; i<argc; i++){
		if(!strcmp(argv[i], "-c") || !strcmp(argv[i], "--config-file")){
            ...
            i++;
        }else if(!strcmp(argv[i], "-distr")){
            if(!strcmp(argv[i+1], "true")) {
                is_distributed_pub = true;
                is_distributed_sub = true;
            }else{
                is_distributed_pub = false;
                is_distributed_sub = false;
            }
            i++;
